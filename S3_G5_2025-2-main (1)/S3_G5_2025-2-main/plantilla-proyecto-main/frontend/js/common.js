        ...
        if (isConnected) {
            ...
            statusDiv.innerHTML = `
                <div class="alert alert-danger">
                    ...
                    <small>Asegúrate de que el backend esté corriendo en <strong>http://localhost:8080</strong></small>
                </div>
            `;
        }
    }
...
async function makeRequest(url, options = {}) {
    ...
    try {
        ...
        // Mostrar mensaje más descriptivo dependiendo del tipo de error
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
            showResult('No se puede conectar al servidor. Asegúrate de que el backend esté corriendo en http://localhost:8080', true);
        } else {
            showResult(`Error al conectar con el servidor: ${error.message}`, true);
        }
        throw error;
    } finally {
        showLoading(false);
